...
EXP_NAME="type1_v2_pairwise_margin_smoke_20260712"
...
# The default cell (pairwise_weight=1.0, margin=0.0) is the v2/unreacted baseline.
CONFIG_NAMES=(
  "pw05_m000"
  "pw05_m005"
  "pw05_m010"
...
PAIRWISE_WEIGHTS=(0.5 0.5 0.5 1.0 1.0 2.0 2.0 2.0)
MARGINS=(0.0 0.05 0.10 0.05 0.10 0.0 0.05 0.10)

for idx in "${!CONFIG_NAMES[@]}"; do
  config_name="${CONFIG_NAMES[$idx]}"
  pairwise_weight="${PAIRWISE_WEIGHTS[$idx]}"
  margin="${MARGINS[$idx]}"
  ...
  for seed in "${SEEDS[@]}"; do
    ...
    echo "Starting pairwise/margin smoke config=$config_name weight=$pairwise_weight margin=$margin seed=$seed on GPU $GPU..."
    mkdir -p "$seed_out"
...
cat > "$OUT_ROOT/matrix_manifest.json" <<JSON
{
  "experiment": "$EXP_NAME",
  "seeds": $(
    printf '['
  ...
  "baseline_default_cell": {
    "pairwise_weight": 1.0,
    "margin": 0.0,
    "path": "$ROOT/results/type1_unreacted_substrate_supplement_v2_20260711"
  },
  "configs": [
$(for idx in "${!CONFIG_NAMES[@]}"; do
    comma=","
    if [ "$idx" -eq "$((${#CONFIG_NAMES[@]} - 1))" ]; then
      comma=""
    fi
    printf '    {"name": "%s", "pairwise_weight": %s, "margin": %s}%s\n' "${CONFIG_NAMES[$idx]}" "${PAIRWISE_WEIGHTS[$idx]}" "${MARGINS[$idx]}" "$comma"
  done)
  ]
}
JSON

echo "pairwise-weight/margin smoke complete."
