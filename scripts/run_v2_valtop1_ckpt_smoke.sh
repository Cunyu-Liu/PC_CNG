...
EXP_NAME="type1_v2_valtop1_ckpt_smoke_20260712"
...
for seed in "${SEEDS[@]}"; do
  seed_name="v2_valtop1_ckpt_pairwise_seed${seed}"
  ...
  echo "Starting val_top1 checkpoint seed $seed on GPU $GPU..."
  mkdir -p "$seed_out"
...
SUMMARY_DIR="$OUT_ROOT/valtop1_ckpt_smoke_summary"
mkdir -p "$SUMMARY_DIR"
PYTHONPATH=. "$PY" -m pc_cng.multiseed_summary \
  --exp-dir "$OUT_ROOT" \
  --prefix v2_valtop1_ckpt_pairwise_seed \
  --seeds "$SEED_CSV" \
...
for seed in "${SEEDS[@]}"; do
  ...
  SIG_ARGS+=(--candidate "$OUT_ROOT/v2_valtop1_ckpt_pairwise_seed${seed}/rerank_same_split/candidate_scores.csv")
done

PYTHONPATH=. "$PY" -m pc_cng.multiseed_paired_significance \
  "${SIG_ARGS[@]}" \
  --baseline-name v2_filtered_3seed \
  --candidate-name valtop1_ckpt_3seed \
  --output-dir "$OUT_ROOT/paired_significance_v2_vs_valtop1_ckpt_smoke" \
  --bootstrap-iterations 5000 \
  --seed 20260712

echo "val_top1 checkpoint smoke complete."
