...
from .train_reaction_center_edit_decoder import EditDecoderMLP, decoder_head_scores
...
def load_decoder(path: str, device: torch.device):
    ...
    model = EditDecoderMLP(len(FEATURE_NAMES), hidden_dim=hidden_dim, dropout=dropout, output_dim=output_dim).to(device)
...
def score_rows(model, rows: List[Dict[str, object]], device: torch.device) -> List[float]:
    ...
    with torch.no_grad():
        return decoder_head_scores(model, x, head="hard_negative").detach().cpu().numpy().tolist()
