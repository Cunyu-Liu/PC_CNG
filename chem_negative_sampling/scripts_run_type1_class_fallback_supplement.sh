...
RESULTS_DIR=${RESULTS_DIR:-$ROOT/results/type1_class_fallback_supplement_20260711}
...
FALLBACK_RAW="$RESULTS_DIR/class_fallback_candidates.csv"
FALLBACK_SUMMARY="$RESULTS_DIR/class_fallback_summary.json"
FALLBACK_REVIEWED="$RESULTS_DIR/class_fallback_candidates_reviewed.csv"
FALLBACK_REVIEW_SUMMARY="$RESULTS_DIR/class_fallback_review_summary.json"
OUT_DIR=${OUT_DIR:-$RESULTS_DIR/fallback_augmented_pairwise_seed20260710}
SEED=${SEED:-20260710}
...
if [[ ! -f "$FALLBACK_SUMMARY" ]]; then
  echo "[generate] class fallback candidates"
  PYTHONPATH=. "$PYTHON_BIN" -m pc_cng.run_hard_negative_actions \
    ...
    --progress-every 1000 \
    > "$LOG_DIR/type1_class_fallback_supplement_generate.log" 2>&1
fi

if [[ ! -f "$FALLBACK_REVIEW_SUMMARY" ]]; then
  echo "[review] class fallback candidates"
  PYTHONPATH=. "$PYTHON_BIN" -m pc_cng.false_negative_review \
    --input "$FALLBACK_RAW" \
    --output "$FALLBACK_REVIEWED" \
    --summary "$FALLBACK_REVIEW_SUMMARY" \
    --known-positive "$HITEA_ALIGNMENT" \
    --known-positive "$REGIO_ALIGNMENT" \
    > "$LOG_DIR/type1_class_fallback_supplement_review.log" 2>&1
fi

if [[ ! -f "$OUT_DIR/metrics.json" ]]; then
  echo "[train] fallback augmented pairwise seed=$SEED"
  rm -rf "$OUT_DIR"
  CUDA_VISIBLE_DEVICES="$GPU_TRAIN" PYTHONPATH=. "$PYTHON_BIN" -m pc_cng.train_pairwise_reward_mlp \
    ...
    --seed "$SEED" \
    > "$LOG_DIR/type1_class_fallback_augmented_pairwise_train.log" 2>&1
fi

if [[ ! -f "$OUT_DIR/rerank_same_split/ranking_metrics.json" ]]; then
  echo "[rerank] fallback augmented pairwise seed=$SEED"
  CUDA_VISIBLE_DEVICES="$GPU_EVAL" PYTHONPATH=. "$PYTHON_BIN" -m pc_cng.evaluate_candidate_reranking \
    ...
    --batch-size "${BATCH_SIZE:-4096}" \
    --device cuda \
    > "$LOG_DIR/type1_class_fallback_augmented_pairwise_rerank.log" 2>&1
fi

if [[ ! -f "$RESULTS_DIR/reaction_class_fallback_trained/reaction_class_benchmark.json" ]]; then
  echo "[class] fallback trained reaction-class benchmark"
  PYTHONPATH=. "$PYTHON_BIN" -m pc_cng.analyze_reaction_class_benchmark \
    --score-csv "fallback_trained=$OUT_DIR/rerank_same_split/candidate_scores.csv:score" \
    --output-dir "$RESULTS_DIR/reaction_class_fallback_trained" \
    --min-groups 20 \
    --weak-top1 0.80 \
    --weak-mrr 0.85 \
    > "$LOG_DIR/type1_class_fallback_augmented_pairwise_class.log" 2>&1
fi

echo "Class-fallback augmented pairwise experiment complete: $OUT_DIR"
