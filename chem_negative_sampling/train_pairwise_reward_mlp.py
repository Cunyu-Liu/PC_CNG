...
def main() -> None:
    ...
    pair_loader = DataLoader(
        TensorDataset(torch.tensor(x_pair_pos, dtype=torch.float32), torch.tensor(x_pair_neg, dtype=torch.float32)),
        batch_size=args.batch_size,
        shuffle=True,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (batch_x, batch_y) in enumerate(supervised_loader):
            ...
            pair_pos, pair_neg = pair_batches[step % len(pair_batches)]
            ...
            pairwise_loss = torch.nn.functional.softplus(args.margin - (pos_score - neg_score)).mean()
        ...
        val_key = val_metrics.get("roc_auc", 0.0)
