...
| val Top-1 checkpoint-selection smoke queue | `/home/cunyuliu/pc_cng_research/results/logs/type1_v2_valtop1_ckpt_smoke_20260712.queue.log`, PID `1688762` |
| representation-scale smoke script | `/home/cunyuliu/pc_cng_research/run_v2_representation_scale_smoke.sh` |
| representation-scale smoke queue | `/home/cunyuliu/pc_cng_research/results/logs/type1_v2_representation_scale_smoke_20260712.queue.log`, PID `1751351`, waits for val_top1 smoke first |
| pairwise-weight/margin smoke script | `/home/cunyuliu/pc_cng_research/run_v2_pairwise_margin_smoke.sh` |
| pairwise-weight/margin smoke queue | `/home/cunyuliu/pc_cng_research/results/logs/type1_v2_pairwise_margin_smoke_20260712.queue.log`, PID `1877389`, waits for representation-scale smoke first |
...
| filtered v2 same-data baseline queue | `/home/cunyuliu/pc_cng_research/results/logs/type1_v2_filtered_baseline_20260712.queue.log`, PID `2312365`; GPU5 first in serial queue |
...
| S2 | 2026-07-13 | Early stopping / checkpoint selection by val Top-1 | 3 seeds smoke, then 10 seeds | val/test ranking alignment improves | implemented and queued on GPU 5 watcher, PID `1688762` |
| S3 | 2026-07-13 to 2026-07-14 | n_bits=8192 and binary_count features | 3 seeds per config | >=+0.5 pp test Top-1 over v2 in smoke | script ready; queued behind val_top1 smoke, PID `1751351` |
...
5. Start cosine LR + warmup 10-seed experiment as soon as GPU 4 is free. **QUEUED**
   - Deliverable: `results/type1_v2_coslr_warm5_20260712/`
   - Acceptance: first seed completes training + rerank without error.
   - Current queue: `/home/cunyuliu/pc_cng_research/results/logs/type1_v2_coslr_warm5_20260712.queue.log`, PID `1512519`.
...
1. Implement or run checkpoint-selection branch based on val Top-1 instead of val ROC-AUC. **IMPLEMENTED + QUEUED**
   ...
   - Current smoke: `results/type1_v2_valtop1_ckpt_smoke_20260712/`, queue PID `1688762`, GPU 5 watcher.
2. Run n_bits=8192 Morgan smoke test on 3 seeds. **SCRIPT READY + QUEUED**
   - Acceptance: if mean test Top-1 >=87.7%, promote to 10 seeds.
   - Queue: `results/type1_v2_representation_scale_smoke_20260712/`, PID `1751351`; waits for val_top1 smoke.
3. Run binary_count Morgan smoke test on 3 seeds. **SCRIPT READY + QUEUED**
   - Acceptance: if no featurization/memory issue and test improves, promote.
   - Queue: same representation-scale smoke job as n_bits=8192.
4. Run pairwise-weight / margin smoke matrix: **SCRIPT READY + QUEUED**
   - `pairwise_weight`: 0.5, 1.0, 2.0
   - `margin`: 0.0, 0.05, 0.10
   - Acceptance: select at most two configs for 10-seed confirmation.
   - Queue: `results/type1_v2_pairwise_margin_smoke_20260712/`, PID `1877389`; default cell uses v2 baseline, 8 non-default cells will run after representation-scale smoke.
