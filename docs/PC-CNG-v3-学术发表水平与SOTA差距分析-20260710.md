...
As of 2026-07-12 20:28, LR scheduling is queued on GPU 4 and validation Top-1
checkpoint selection is implemented and queued as a 3-seed smoke on GPU 5.
As of 2026-07-12 20:35, representation-scale smoke is queued behind the
val_top1 smoke: Morgan n_bits=8192 and binary_count Morgan 4096 will each run
3 seeds before any 10-seed promotion decision.
As of 2026-07-12 20:40, the dedicated benchmark manifest is created:
PC-CNG-v3-benchmark-manifest-20260712.md.
As of 2026-07-12 20:47, the pairwise_weight/margin objective smoke matrix is
scripted and queued behind representation-scale smoke.
As of 2026-07-12 21:15, manifest data-quality audit is pass_with_warnings:
10 diverse-anchor keep negatives overlapping known positive products were
removed into a filtered CSV used by all still-queued experiments. If any
filtered-data branch is promoted, rerun v2 on the same filtered CSV before
making final same-data claims.
As of 2026-07-12 21:32, filtered v2 10-seed baseline rerun is queued first on
GPU5, and downstream smoke scripts use that baseline for paired significance.
The manifest audit/filter path is now unit-tested locally and remotely.
As of 2026-07-12 21:50, original benchmark support audit quantifies the current
same-context test support at 111 combined test groups (43 real reactant groups
+ 68 synthetic source groups), leaving an 89-group deficit to the M3 >=200
target.
As of 2026-07-12 22:32, the USPTO/OpenMolecules M3 expansion scan found 51,494
eligible held-out test reactant contexts after split/context/reaction leakage
filters and selected 256 positive parents for negative generation/review. This
is a sufficient parent pool for closing the deficit, but it is not yet an
evaluable benchmark expansion until boundary negatives, known-positive
filtering, review-status assignment, and support re-audit are complete.
As of 2026-07-12 23:13, that M3 expansion has passed the support gate:
1,746 raw candidates were generated for the 256 selected USPTO parents, the
reviewed/known-positive-filtered synthetic CSV retains 1,616 rows with 1,268
`keep_synthetic_negative` rows, and the integrated support audit reports 323
combined test groups against the >=200 target. This closes the original
held-out test-size/variance gate, but does not close the original held-out
Top-1/MRR/NDCG SOTA performance gates.
As of 2026-07-12 23:30, the first expanded-benchmark v2/unreacted baseline is
measured: 10-seed mean Test Top-1 is 51.60 ± 0.89% over 293 scored/evaluable
test groups, with ensemble Test Top-1 51.88%. The USPTO expansion subset has
212 scored groups and only 38.02 ± 1.54% Top-1. This is not comparable to the
old 111-group original held-out metric as a simple regression; it is a harder,
variance-reduced benchmark that exposes a new data-scale generalization gap.
As of 2026-07-12 23:45, hidden4096 and dropout04 were retested on this expanded
benchmark. hidden4096 has a small Top-1 trend (52.12 ± 0.85%, ensemble ΔTop-1
+0.48 pp), but paired CI crosses zero [-0.07,+1.03] pp and p=0.143. dropout04
remains null/negative (ensemble ΔTop-1 -0.14 pp, p=0.626). Thus capacity/dropout
alone remains rejected as the main SOTA route even under the harder expanded
benchmark.
As of 2026-07-13 00:16, combined Morgan+graph_stats was also retested on the
expanded M3 benchmark. Its 10-seed mean Test Top-1 is slightly higher
(52.42 ± 0.84%), but paired ensemble ΔTop-1 is only +0.14 pp with CI
[-0.62,+0.89] pp and p=0.864. Therefore combined remains a strong
expanded-curated / weak-class architecture supplement, not an expanded M3 main
candidate.
As of 2026-07-13 10:05, classw050_rc was retested on the expanded M3 benchmark.
It slightly improves USPTO subset mean Top-1 (40.38 ± 1.67%), but paired
ensemble ΔTop-1 on the full expanded benchmark is only +0.07 pp with CI
[-0.89,+1.03] pp and p=1.000; MRR/NDCG are not improved. Therefore classw050_rc
remains a weak-class supplement, not an expanded M3 main candidate.
```
...
### Ni external data-source gap audit (updated 2026-07-13)

Using `pc_cng/audit_ni_atomic_support.py`, with a fast `Ni` text prefilter
followed by RDKit atomic-number detection (atomic number 28), across the current
processed HITEA and USPTO/OpenMolecules normalized datasets:

| Dataset | Total reactions | Ni reactions | Distinct Ni parent reactants |
|---|---:|---:|---:|
| HITEA normalized | 39,546 | 0 | 0 |
...
| Unit test | `/home/cunyuliu/pc_cng_research/chem_negative_sampling/tests/test_ni_atomic_support_audit.py`; remote passed with `rdkit_available=true` |
